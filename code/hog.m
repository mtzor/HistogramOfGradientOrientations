...
for i = 1:nori
  ...
  ormin=(-pi/2)+(i-1)*pi/9;
  ormax=(-pi/2)+i*pi/9;
  B = (mag>=thresh)&(ormin<ori)&(ori<=ormax);
...
divider=sum(ohist,3);%finding sum to normalise
zero=(divider==0);%finding where sum is zero
divider_no_zero=zero+divider;%adding 1 to the zeros so we can divide

ohist= bsxfun(@rdivide, ohist, divider_no_zero);
%nohist=ohist./divider
